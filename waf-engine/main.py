...
app.state.threat_intel = ThreatIntelService(app.state.redis)
...
_ADMIN_USERS = {
    "admin": hashlib.sha256("AegisX@2026".encode()).hexdigest(),
    "operator": hashlib.sha256("Waf0p3r@tor".encode()).hexdigest(),
}
...
async def login(credentials: LoginRequest):
    ...
    if username not in _ADMIN_USERS or _ADMIN_USERS[username] != password_hash:
    ...
    await app.state.redis.set_with_ttl(f"auth:token:{token}", token_data, ttl=86400)
...
async def verify_token(request: Request):
    ...
    token_data = await app.state.redis.get_value(f"auth:token:{token}")
...
async def logout(request: Request):
    ...
    if auth_header.startswith("Bearer "):
        token = auth_header.replace("Bearer ", "")
        await app.state.redis.delete_key(f"auth:token:{token}")
...
async def analyze_request(request: RequestMetadata):
    ...
    try:
        ...
        # 1. Apply Deterministic OWASP CRS Regex Engine
        regex_result = app.state.regex_engine.analyze(
            method=request.method,
            uri=request.uri,
        ...
        if regex_result["matched"]:
            ...
            decision = {
                ...
                "blocked_by": "RegexEngine",
                "latency_ms": latency_ms
            }
        else:
            # 2. Get AI decision (Fallback/Anomaly Detection)
