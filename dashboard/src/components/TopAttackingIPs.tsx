...
export default function TopAttackingIPs() {
    ...
    const fetchTopIPs = async () => {
        try {
            ...
            const mappedIps = data.map((item: any) => ({
                ip: item.source_ip,
                country: item.geo_country || 'Unknown',
                country_code: item.geo_country || 'XX',
                request_count: item.request_count,
                threat_level: item.request_count > 100 ? 'critical' : item.request_count > 50 ? 'high' : item.request_count > 10 ? 'medium' : 'low'
            }));
            setIps(mappedIps.slice(0, 10));
        } catch (error) {
            // Mock data for demo
            setIps([
